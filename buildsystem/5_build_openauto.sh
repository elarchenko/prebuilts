...
if [ -z "${OPENAUTO_GIT_REPO}" ]; then
  OPENAUTO_GIT_REPO='https://github.com/elarchenko/openauto.git'
fi
# Install Pre-reqs
...
cmake -DCMAKE_BUILD_TYPE=Release -DRPI3_BUILD=TRUE -DAASDK_INCLUDE_DIR="/usr/local/include/aasdk" -DAASDK_LIBRARIES="/usr/local/lib/libaasdk.so" -DAASDK_PROTO_INCLUDE_DIRS="/usr/local/include/aasdk_proto" -DAASDK_PROTO_LIBRARIES="/usr/local/lib/libaasdk_proto.so" ../openauto
